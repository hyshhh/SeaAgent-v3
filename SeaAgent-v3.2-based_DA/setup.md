...
系统只有一个主智能体。Skills 先提供名称和描述（每一轮的提示词里都有目录），模型需要时用 `read_file` 读取 `/skills/<name>/SKILL.md` 正文；如果整段会话一次都没读过正文，中间件会在模型动手前提醒一次。文件读取权限收敛在 skills 目录内，写入与 shell 工具保持禁用。工具调用次数、重试、摘要和上下文边界由配置及官方 Middleware 管理；模型轮次不设上限，收尾时机由 `skills/finalize` 与证据守卫共同约束。页面只显示公开状态、Skill、模型、工具、证据和最终消息，不显示隐藏推理。
