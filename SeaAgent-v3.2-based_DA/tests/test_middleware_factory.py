...
def test_official_middleware_stack():
    ...
    assert [type(item) for item in stack] == [SkillDisclosureMiddleware, SummarizationMiddleware, ToolCallLimitMiddleware, ModelRetryMiddleware, ToolRetryMiddleware, TodoListMiddleware, EvidenceWrapUpMiddleware]
...
def test_disclosure_guard_reads_its_settings_from_config():
    config = load_config()
    guard = build_middleware(config, FakeListChatModel(responses=["ok"]))[0]
