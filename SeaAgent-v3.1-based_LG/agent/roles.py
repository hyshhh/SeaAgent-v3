...
def role_system_prompt(agent_key: str, title: str, responsibility: str) -> str:
    ...
    work_style = (
        ...
        "- 完成后调用对应的提交工具（submit_* / request_replan）结束本轮。\n"
        "- 只使用分配给你的工具，不要编造工具结果。\n"
        "- 回答与 reason 使用简体中文。\n"
    )
    if agent_key == "plan_agent":
        work_style = (
            ...
            "- 随后必须调用 submit_plan(goal, calls, planHint)；确实无法形成计划时才调用 submit_observation。\n"
        ...
        work_style = (
            ...
            "- 审阅后必须调用 submit_observation(summary, evidenceGap, proposedState)。\n"
        )
        ...
        work_style = (
            ...
            "- pendingRequirements 非空且未达轮次上限时，立即调用 request_replan。\n"
            "- 只有 acceptanceSatisfied=true，或继续检索已无收益时，才允许调用 submit_verdict。\n"
...
INTENT_RESPONSIBILITY = (
    ...
    "完成后调用 submit_intent，arguments 中携带结构化意图。"
)

PLAN_RESPONSIBILITY = (
    "根据意图规划本轮最小 calls（含 $ref），然后立即调用 submit_plan；"
    "不要执行业务检索工具，不要长篇解释；无法规划时才 submit_observation。"
)

OBSERVE_RESPONSIBILITY = (
    "审阅确定性执行器产生的工具摘要与证据域，按需读取观察技能；"
    "完成后调用 submit_observation，summary 中只写可核验事实与真实缺口。"
)

REFLECT_RESPONSIBILITY = (
    "以 acceptanceProgress 为权威审计证据是否充分，按需读取验收技能；"
    "数据库问题不得扩展到视频域，视频与全库对照问题则按各自验收清单决定是否再规划；"
    "需要下一轮时调用 request_replan，否则调用 submit_verdict。"
)
